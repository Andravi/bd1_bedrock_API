...
class Universo(db.Model):
    ...
    @classmethod
    def contar_personagens(cls, univ_nome):
        return (
            db.session.query(func.count(Anotacao.titulo))
            .join(TipoAnotacao, Anotacao.tipo_anota == TipoAnotacao.nome)
            .filter(Anotacao.nuniverso == univ_nome, TipoAnotacao.nome == "Personagem")
            .scalar()
            or 0
        )
...
class Categoria(db.Model):
    ...
    anotacoes = relationship(
        "Anotacao", secondary="catego_anotacao", back_populates="categorias"
    )
...
class Anotacao(db.Model):
    ...
    categorias = relationship(
        "Categoria", secondary="catego_anotacao", back_populates="anotacoes"
    )
    ...
    mencionadas = relationship(
        "Mencao", foreign_keys="Mencao.anot_mencionando", back_populates="mencionando"
    )
    mencionando = relationship(
        "Mencao", foreign_keys="Mencao.anot_mencionada", back_populates="mencionada"
    )
...
class CategoAnotacao(db.Model):
    ...
    ncategoria = db.Column(
        db.String(100), db.ForeignKey("categoria.nome"), primary_key=True
    )
    nanotacao = db.Column(
        db.String(100), db.ForeignKey("anotacao.titulo"), primary_key=True
    )
...
class Coluna(db.Model):
    ...
    titulo = db.Column(db.String(100), primary_key=True)
    ttabela = db.Column(
        db.String(100), db.ForeignKey("tabela.titulo"), primary_key=True
    )
...
class Lugar(db.Model):
    ...
    nome = db.Column(
        db.String(100), db.ForeignKey("tipo_anotacao.nome"), primary_key=True
    )
...
class Propriedade(db.Model):
    ...
    localizacao = relationship(
        "Localizacao", uselist=False, back_populates="propriedade"
    )
...
class Localizacao(db.Model):
    ...
    nprop = db.Column(
        db.String(100), db.ForeignKey("propriedade.nome"), primary_key=True
    )
...
class Mencao(db.Model):
    ...
    anot_mencionada = db.Column(
        db.String(100), db.ForeignKey("anotacao.titulo"), primary_key=True
    )
    anot_mencionando = db.Column(
        db.String(100), db.ForeignKey("anotacao.titulo"), primary_key=True
    )
    palavra_chave = db.Column(db.String(100))
    posicao_no_texto = db.Column(db.Integer)

    mencionada = relationship(
        "Anotacao", foreign_keys=[anot_mencionada], back_populates="mencionando"
    )
    mencionando = relationship(
        "Anotacao", foreign_keys=[anot_mencionando], back_populates="mencionadas"
    )
